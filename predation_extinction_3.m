function predation_extinction_3
global p q y L T
pp = [0.01 0.1 1 10 100 1000 10000];  % predation rate
...
a = zeros(1,length(pp)); % Three species coexist 三物种共存
b = zeros(1,length(pp)); % Two species extinct, one species surviving 两物种灭绝，一物种存活
E = zeros(1,length(pp)); % extinction probability 灭绝概率
...
xlabel({'Predation, p'},'FontWeight','bold','FontSize',14);  %X轴名称及字体和大小 
